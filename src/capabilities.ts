...
export function parseCapabilities(value: unknown): Capabilities {
  ...
  for(const key of ['agents.parallel','agents.runs','agents.catalog','agents.plans','episodes.forget','documents.files','documents.provenance','processing.distill','processing.derive','graph.knowledge','entities.read','graph.report','graph.path','graph.export','graph.sources','graph.timeline','graph.knowledge_paging','graph.knowledge_seeds','graph.knowledge_walk','recall.graph_boost']){
