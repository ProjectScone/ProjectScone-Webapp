...
type OptionalFeature = 'graph.path' | 'graph.knowledge' | 'entities.read' | 'graph.report' | 'images.understand' | 'models.manage' | 'episodes.attachments' | 'episodes.list' | 'episodes.read' | 'facts.links' | 'integrity.read' | 'profile.read' | 'processing.distill' | 'processing.derive' | 'jobs.read' | 'recall.conditions';
...
export function parseCapabilities(value: unknown): Capabilities {
  ...
  for(const key of ['processing.distill','processing.derive','graph.knowledge','entities.read','graph.report','graph.path']){
