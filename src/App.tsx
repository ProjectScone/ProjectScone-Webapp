...
import {SourceRemovalPage} from './memory/SourceRemovalPage';
...
export function App({ initialKey }: { initialKey: string }) {
  ...
  return <BrowserRouter>
    ...
    <Routes>
      ...
      <Route path="/memory/sources/:id/forget" element={<SourceRemovalPage key={epoch} api={api} enabled={Boolean(key)}/>} />
    ...
    {auth && <Modal title="Memory connection" onClose={() => setAuth(false)}>
      ...
      <p className="setup-intro">Use a <strong>Scone space key</strong> from this server’s configuration or administrator—not a Claude, Codex or OpenAI API key. A single-key preview supplies it automatically.</p>
