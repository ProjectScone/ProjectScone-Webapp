...
export type GraphLayout = 'constellation' | 'radial' | 'flow' | 'growth';
const options: {id:GraphLayout;name:string;description:string}[] = [
  ...
  {id:'growth',name:'Growth spiral',description:'Read an agent’s recorded life, event by event.'},
];
function Preview({mode}:{mode:GraphLayout}) {
  const points=mode==='growth'?Array.from({length:16},(_,i)=>{const angle=i*2.39996,r=Math.sqrt(i)*5;return[31+Math.cos(angle)*r,25+Math.sin(angle)*r];}):mode==='flow'?[[9,15],[31,15],[53,15],[31,35],[53,35]]:mode==='radial'?[[31,25],[31,5],[53,25],[31,45],[9,25]]:[[25,23],[12,7],[51,12],[48,40],[8,41]];
