...
export function PlaygroundPage({ api, enabled }: { api: ApiClient; enabled: boolean }) {
  ...
  return <div className="workspace">
    <details className="explorer-session-switcher"><summary>Activity scope ⌄</summary><aside className="rail" aria-label="Sessions"><div className="rail-head"><h2>Session trace</h2><p className="muted">Interactions and the evidence they leave behind.</p></div><section><div className="rail-label"><span className="eyebrow">Recorded sessions</span><span>{sessions.length}</span></div><div id="sessions">
    ...
    </div></section><div className="rail-footer"><span className="eyebrow">Evidence, not inference</span><p>A captured conversation is a source, not an approved belief. Connections require stored references.</p></div></aside></details>
    <main className="main" id="main"><WorkspaceHeading className="heading" eyebrow="The memory atlas" title="Memory atlas" description="" actions={<div className="heading-controls"><span className="pill"><span className={`dot ${updated && !error ? 'online' : ''}`} /><span id="connection" role="status">{!enabled ? 'Connection needs authentication' : error || (updated ? `API online · ${sessions.length ? 'activity recorded' : 'No agent events'}` : 'Connecting to Scone API…')}</span></span><button onClick={() => setConnections(true)}>Connect agents</button><button aria-label={paused ? 'Resume live updates' : 'Pause live updates'} onClick={() => setPaused(value => !value)}>{paused ? '▶ Resume' : 'Ⅱ Pause'}</button></div>}/>
