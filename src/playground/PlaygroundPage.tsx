...
export function PlaygroundPage({ api, enabled }: { api: ApiClient; enabled: boolean }) {
  ...
  const { graph, error, updated } = useEvidence(api, enabled, paused, revision);
  ...
  return <div className="workspace">
    <aside className="rail" aria-label="Sessions"><div className="rail-head"><h2>Session trace</h2><p className="muted">Interactions and the evidence they leave behind.</p></div><section><div className="rail-label"><span className="eyebrow">Recorded sessions</span><span>{sessions.length}</span></div><div id="sessions">
      <button className={`session ${!session ? 'selected' : ''}`} aria-pressed={!session} onClick={() => { setSession(null); setSelected(null); }}><b>All recorded activity</b><small>{graph.nodes.length} records in this snapshot</small></button>
      {sessions.map(node => <button key={node.id} className={`session ${session === node.id ? 'selected' : ''}`} aria-pressed={session === node.id} onClick={() => { setSession(node.id); setSelected(null); }}><b>{node.data?.agent === 'claude-code' ? 'Claude Code' : node.data?.agent === 'codex' ? 'Codex' : node.label}</b><small>{String(node.data?.project || 'Session')} · {String(node.data?.session_id || node.id).slice(0,8)}</small></button>)}
    ...
    <main className="main" id="main"><div className="heading"><div><h1>Playground</h1><p className="muted">Trace an interaction. Inspect its memory. Test its recall.</p></div><div className="heading-controls"><span className="pill"><span className={`dot ${updated && !error ? 'online' : ''}`} /><span id="connection" role="status">{!enabled ? 'Connection needs authentication' : error || (updated ? `API online · ${sessions.length ? 'activity recorded' : 'No agent events'}` : 'Connecting to local API…')}</span></span><button onClick={() => setConnections(true)}>Connect agents</button><button aria-label={paused ? 'Resume live updates' : 'Pause live updates'} onClick={() => setPaused(value => !value)}>{paused ? '▶ Resume' : 'Ⅱ Pause'}</button></div></div>
      ...
      <section className={`stage evidence-atlas ${inspected ? 'has-inspector' : 'map-only'}`} aria-label="Evidence explorer"><div className={`graph-panel ${layout}-theme ${records ? 'records-mode' : ''}`}>
        <div className="graph-top"><span className="graph-title">{visible.length} records · {edges.length} links</span><span id="coverage">{error && graph.nodes.length ? 'Stale snapshot · not live' : graph.truncated ? 'Partial snapshot · records omitted' : ''}</span>{layout!=='growth'&&<div className="view-switch"><button aria-label="2D view" aria-pressed={!depth} onClick={() => setDepth(false)}>2D</button><button aria-label="Depth view" aria-pressed={depth} onClick={() => setDepth(true)}>Depth</button></div>}</div>
        {depth && layout!=='growth'&&<div id="layout-note">Layered view · depth is not confidence</div>}
      ...
      </div>{inspected && <SourceInspector node={inspected} graph={graph} select={setSelected} close={() => setSelected(null)} />}</section>
      ...
      </section><footer className="coverage-bar"><span>Source <b>authenticated local API</b></span><span>Capture <b>{sessions.length ? 'activity recorded; liveness unverified' : 'no agent events'}</b></span><DevReload /><span>Bounded snapshot · not a complete replay</span></footer>
    </main>
