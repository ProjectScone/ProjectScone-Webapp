...
function Editor({api,space,catalog,initial,onSave,onDirty,inputsAvailable,requirementsAvailable,schemaAvailable}:{api:ApiClient;space:string;catalog:AgentChoice[];initial:SavedPlan|null;onSave:(plan:SavedPlan)=>void;onDirty:()=>void;inputsAvailable:boolean;requirementsAvailable:boolean;schemaAvailable:boolean}){
 ...
 return <form className="agent-editor" noValidate onChange={onDirty} onSubmit={event=>{event.preventDefault();void save();}}>
  ...
  <fieldset disabled={busy}>
   ...
   <div className="workflow-builder"><WorkflowCanvas tasks={plan.tasks} selected={Math.min(selectedTask,plan.tasks.length-1)} onSelect={setSelectedTask}/><div className="workflow-inspector">{plan.tasks.map((task,index)=>{
    const agent=isInputTask(task)?undefined:catalog.find(choice=>choice.agent_id===task.agent_id);
    return <section className="agent-task" hidden={index!==Math.min(selectedTask,plan.tasks.length-1)} key={taskKeys[index]} aria-label={`Task ${index+1}`}>
...
export function AgentsPage({api,enabled}:{api:ApiClient;enabled:boolean}){
 const [workspaceMode,setWorkspaceMode]=useState<'build'|'run'>('build');
 ...
 return <main id="main" className="agents-page"><WorkspaceHeading eyebrow="The control room" title="Workflows" description="Build, run, and inspect."/>
  {!enabled?<WorkspaceState icon="scopes" title="Connect a memory space" description="Use Memory connection to access saved workflows."/>:loading?<WorkspaceState icon="scopes" title="Loading workflows" description="Checking this host’s agent configuration." busy/>:!data?<WorkspaceState icon="scopes" title="Workflows unavailable" description={error} actions={<button onClick={()=>setRefresh(n=>n+1)}>Try again</button>}/>:<>
   ...
   })()}</div>{data.runsAvailable&&<div hidden={workspaceMode!=='run'}><RunPanel key={data.space} api={api} space={data.space} historyAvailable={data.historyAvailable} textStreamAvailable={data.textStreamAvailable} plan={selection.plan} dirty={dirty} maxParallel={data.maxParallel} handoffsAvailable={data.handoffsAvailable} inputsAvailable={data.inputsAvailable} usageAvailable={data.usageAvailable} approvalsAvailable={data.approvalsAvailable}/></div>} {error&&<p role="alert">{error}</p>}
  </>}
 </main>;
}
