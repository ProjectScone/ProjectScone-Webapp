...
function RunView({api,space,id,expected,onChanged,inputsAvailable,usageAvailable,approvalsAvailable,historyAvailable,textStreamAvailable,approvalAttempt}:{api:ApiClient;space:string;id:string;expected?:RunSubmission;approvalAttempt:ApprovalAttemptRef;onChanged:()=>void;inputsAvailable:boolean;usageAvailable:boolean;approvalsAvailable:boolean;historyAvailable:boolean;textStreamAvailable:boolean}){
 ...
 return <section className="agent-run-view" aria-label="Selected run"><h3>Run {id}</h3>
  ...
  {request&&status&&textStreamAvailable&&!result&&(status.active_local||status.status==='running')&&status.inflight_steps.map(step=><AnswerPane key={step} api={api} runId={id} stepId={step} onTerminal={()=>setVersion(n=>n+1)}/>)}
...
export function RunPanel({api,space,plan,dirty,maxParallel,handoffsAvailable,inputsAvailable,usageAvailable,approvalsAvailable,historyAvailable=false,textStreamAvailable=false}:{api:ApiClient;space:string;plan:SavedPlan|null;dirty:boolean;maxParallel:number;handoffsAvailable:boolean;inputsAvailable:boolean;usageAvailable:boolean;approvalsAvailable:boolean;historyAvailable?:boolean;textStreamAvailable?:boolean}){
 ...
 return <section className="agent-runs" aria-label="Workflow runs"><h2>Run a workflow</h2><p>{plan?`Selected: ${plan.plan.workflow_id} · Revision ${plan.revision}`:'Select or save a workflow to run it.'}</p>
  ...
  {selected&&<RunView approvalAttempt={pendingApproval(selected.id)} key={`${selected.id}:${selected.version}`} api={api} space={space} id={selected.id} expected={selected.expected} inputsAvailable={inputsAvailable} usageAvailable={usageAvailable} approvalsAvailable={approvalsAvailable} historyAvailable={historyAvailable} textStreamAvailable={textStreamAvailable} onChanged={()=>setHistory(n=>n+1)}/>}
