import {parseKnowledgeInference,parseEntityInference,validateInferenceContext,type KnowledgeInference,type EntityInference} from './knowledge-implied.ts';
...
export interface Knowledge {space:string;identity:string;revision:number;mode:KnowledgeMode;asOf:string;entities:Entity[];relations:Relation[];attributes:Attribute[];coverage:Coverage;analysis:KnowledgeAnalysis|null;inference:KnowledgeInference|null}
...
export interface EntityDetail {entity:Entity;follows:EntityInference[];facts:DetailFact[];consistent:boolean;complete:boolean;coverage:Coverage;relations:{predicate:string;direction:'incoming'|'outgoing';other:{id:string;label:string};factIds:number[]}[];attributes:Attribute[]}
...
export function parseKnowledgeCoverage(value:unknown):Coverage {
 ...
 for(const key of ['facts_read','facts_counted','facts_limit','entities_total','entities_shown','relations_total','relations_shown','attributes_total','attributes_shown','follows_total','follows_shown'])if(v[key]!==undefined&&v[key]!==null)counts[key]=num(v[key]);
...
export function parseKnowledge(value:unknown,mode:KnowledgeMode,expectedSpace?:string):Knowledge {
 ...
 const c=parseKnowledgeCoverage(v.coverage),inference=parseKnowledgeInference(v.implied,record(v.coverage),known);
 ...
 if(inference)validateInferenceContext(inference.edges,inference.meanings,relations,mode,str(filters.as_of));
 return {space,identity:projection.key,revision:projection.revision,mode,asOf:str(filters.as_of),entities,relations,attributes,coverage:c,inference,analysis:v.groupings==null?null:parseKnowledgeAnalysis(v.groupings,known)};
}
export function knowledgeNetwork(graph:Knowledge,showImplied=false):{nodes:QueryEvidenceNode[];edges:QueryEvidenceEdge[]} {
 return {nodes:graph.entities.map(e=>({id:e.id,kind:'concept',label:e.label,data:{}})),edges:graph.relations.map(r=>({id:r.id,kind:'relation',source:r.source,target:r.target,data:{predicate:r.predicate,fact_ids:r.factIds}} as QueryEvidenceEdge)).concat(showImplied?(graph.inference?.edges??[]).map(r=>({id:r.id,kind:'implied',source:r.source,target:r.target,data:{predicate:r.predicate,fact_ids:r.factIds}})):[])};
}
export function parseEntityDetail(value:unknown,graph:Knowledge,selected:string):EntityDetail {
 ...
 const knownNames=new Map(graph.entities.map(e=>[e.id,{key:e.key,label:e.label}]));
 const expected=knownNames.get(selected);if(expected&&(expected.key!==selectedEntity.key||expected.label!==selectedEntity.label))throw Error('Entity names changed while inspecting. Refresh the graph.');
 knownNames.set(selected,selectedEntity);
 ...
 const follows=parseEntityInference(v.follows,graph.inference,selected,record(v.coverage),knownNames);
 validateInferenceContext(follows,graph.inference?.meanings??null,graph.relations,graph.mode,graph.asOf);
 const detailCoverage=record(v.coverage);if(detailCoverage.follows_total!==undefined&&(num(detailCoverage.follows_total)<follows.length||num(detailCoverage.follows_total)>follows.length&&detailCoverage.truncated!==true))throw Error('Inconsistent inferred relationship coverage');
 const supported=new Set([...follows.flatMap(r=>r.factIds),...relations.flatMap(r=>r.factIds),...attributes.flatMap(a=>a.factIds)]);
 ...
 return {entity:selectedEntity,relations,attributes,follows,facts,consistent:bool(v.consistent),complete:bool(v.complete),coverage:parseKnowledgeCoverage(v.coverage)};
}
