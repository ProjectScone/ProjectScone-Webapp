...
function TranscriptPlayer({api,source,episodeId,space,media}:Props&{media:MediaTranscript}){
 ...
 return <div><p>{media.segments.length} transcript segments · {transcriptTime(media.durationSeconds)} decoded audio{media.revision?` · ${media.revision}`:''}</p>
  {media.windowing&&<p>{media.windowing.coverage?`${media.windowing.coverage.windows} transcription windows · ${media.windowing.coverage.emptyWindows} returned no text. Empty results do not establish silence or recognition accuracy.`:'Window counts were not recorded for this extraction.'}</p>}
