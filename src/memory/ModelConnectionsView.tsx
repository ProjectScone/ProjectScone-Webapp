...
import {MODEL_ROLES,connectionDraft,connectionFromDraft,readModelConnections,saveModelConnection,probeModelConnection,
  type ModelRole,type ModelConnections,type ConnectionDraft,type ModelProbe,type InferenceProvider} from './model-connections';
...
export function ModelConnectionsView({api}:{api:ApiClient}){
  ...
  const selectProvider=(provider:InferenceProvider)=>{
    cancel();setBusy(null);setProbe(null);setError(null);setNotice(null);
    setDrafts(previous=>({...previous,[role]:{...connectionDraft(null),provider,
      ...(provider==='openrouter'?{base_url:'https://openrouter.ai/api/v1/',model:'google/gemma-4-31b-it',api_key_env:'OPENROUTER_API_KEY'}:{})}}));
  };
  ...
  return <div className="model-connections">
    <header className="model-connections-header"><div className="model-connections-heading"><span className="model-connections-icon"><WorkspaceIcon name="scopes"/></span><div><span className="eyebrow">Your infrastructure</span><h2>Model connections</h2><p>Select local models or explicitly connect cloud inference.</p></div></div><button className="btn quiet" disabled={busy!==null} onClick={()=>void load(Boolean(snapshot))}>{busy==='load'?'Reading settings…':'Refresh saved settings'}</button></header>
      ...
      <section className="model-connection-card" aria-labelledby="model-purpose-heading">
        ...
        <form onSubmit={event=>{event.preventDefault();void save();}}>
          <fieldset disabled={locked}>
            {role==='chat'&&<label>Inference provider<select aria-label="Inference provider" value={draft.provider} onChange={event=>selectProvider(event.target.value==='openrouter'?'openrouter':'self_hosted')}><option value="self_hosted">Self-hosted</option><option value="openrouter">OpenRouter</option></select></label>}
            {draft.provider==='openrouter'&&<p className="model-message">Conversation prompts and retrieved context will be sent to OpenRouter and its model provider. Storage stays on this server.</p>}
            <div className="model-connection-fields"><label>{draft.provider==='openrouter'?'OpenRouter endpoint URL':'Self-hosted endpoint URL'}<input aria-label={draft.provider==='openrouter'?'OpenRouter endpoint URL':'Self-hosted endpoint URL'} type="url" required value={draft.base_url} maxLength={2048} placeholder="http://inference.home.arpa:8080/v1" autoComplete="off" spellCheck={false} onChange={event=>edit('base_url',event.target.value)}/><small>{draft.provider==='openrouter'?'https://openrouter.ai/api/v1/':'The OpenAI-compatible base URL on your machine, edge server, or private network.'}</small></label>
          ...
          {saved&&<details className="model-current"><summary>Saved configuration{changed?' · differs from your draft':''}</summary><dl><div><dt>Provider</dt><dd>{saved.provider==='openrouter'?'OpenRouter':'Self-hosted'}</dd></div><div><dt>Endpoint</dt><dd>{saved.base_url}</dd></div><div><dt>Model</dt><dd>{saved.model}</dd></div><div><dt>Timeout</dt><dd>{saved.timeout_s} seconds</dd></div>{saved.api_key_env&&<div><dt>Token variable</dt><dd>{saved.api_key_env}</dd></div>}{role==='speech'&&<><div><dt>Voice</dt><dd>{saved.voice??'Not set'}</dd></div><div><dt>Sample rate</dt><dd>{saved.sample_rate} Hz</dd></div></>}</dl></details>}
