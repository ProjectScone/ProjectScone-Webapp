...
export interface MediaTranscript {durationSeconds:number;revision?:string;segments:TranscriptSegment[];audio?:DocumentAttachment}
...
export function parseMediaTranscript(format:string,parser:string,metadata:Record<string,unknown>,values:unknown[]):MediaTranscript|undefined{
 ...
 return {durationSeconds,revision,segments,audio};
}
