...
export function ImportEvidence({api,value}:{api:ApiClient;value:VerifiedImport}){
 ...
 return <div className="import-evidence"><p>Source #{receipt.episodeId} · {receipt.segments} extracted {receipt.segments===1?'segment':'segments'} · {receipt.format}{receipt.deduplicated?' · Existing source reused':''}</p>
  ...
  {receipt.videoOcr&&<p>{receipt.segments===0?'No text recognized in sampled frames. The original video and sampling evidence are retained; this source has no searchable text.':'Visible text from sampled video frames. The retained extraction matches your selection.'} Audio was not transcribed.</p>}
