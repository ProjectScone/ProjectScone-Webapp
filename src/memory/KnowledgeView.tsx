import {KnowledgeInferences,EntityInferences} from './KnowledgeInferences';
...
import {moveKnowledgePage,parseKnowledgePage,type KnowledgePage,type KnowledgeTrail} from './knowledge-paging';
...
import {KNOWLEDGE_MODES,knowledgeNetwork,parseEntityDetail,type Coverage,type EntityDetail,type Knowledge,type KnowledgeMode} from './knowledge';
...
export function KnowledgeView({api,detailsAvailable,statusAvailable,analysisAvailable,pathsAvailable,exportsAvailable,timelineAvailable,pagingAvailable,seedsAvailable,walkAvailable}:{api:ApiClient;detailsAvailable:boolean;statusAvailable:boolean;analysisAvailable:boolean;pathsAvailable:boolean;exportsAvailable:boolean;timelineAvailable:boolean;pagingAvailable:boolean;seedsAvailable:boolean;walkAvailable:boolean}){
 ...
 const [inferenceChoice,setInferenceChoice]=useState<{graph:Knowledge;shown:boolean}|null>(null);
 ...
 const context=useMemo(()=>({api,mode,attempt,statusAvailable,analysisEnabled,resolution,pagingAvailable}),[api,mode,attempt,statusAvailable,analysisEnabled,resolution,pagingAvailable]);
 const [navigation,setNavigation]=useState<{context:object;trail:KnowledgeTrail}|null>(null);
 ...
 useEffect(()=>{
  ...
  const params=new URLSearchParams({status:mode,limit:'150',attribute_limit:'1000',...(analysisEnabled?{groupings:'true',resolution:String(resolution)}:{})});
  if(trail){params.set('as_of',trail.anchor.asOf);const cursor=trail.pages[trail.index].cursor;if(cursor)params.set('cursor',cursor);}
  Promise.all([api.request<unknown>('/v1/graph/knowledge?'+params,{signal,cache:'no-store'}),statusAvailable?api.request<unknown>('/v1/status',{signal,cache:'no-store'}).then(verifiedSpace):Promise.resolve(undefined)])
   .then(([value,space])=>parseKnowledgePage(value,mode,space,trail,pagingAvailable)).then(data=>{if(analysisEnabled&&(!data.graph.analysis||(data.graph.analysis.resolution??1)!==resolution))throw Error('The server did not return the requested community analysis settings.');if(!controller.signal.aborted)setSnapshot({request,result:{data}});})
 ...
 const showImplied=Boolean(graph&&inferenceChoice?.graph===graph&&inferenceChoice.shown);
 const network=useMemo(()=>graph?knowledgeNetwork(graph,showImplied):null,[graph,showImplied]);
 ...
 return <section className="knowledge-workspace" aria-label="Knowledge workspace">
  ...
  {!result?<p role="status">Reading this space’s knowledge…</p>:result.error?<div role="alert"><h2>Knowledge could not be loaded</h2><p>{result.error}</p><button className="btn quiet" onClick={()=>setAttempt(value=>value+1)}>Try again</button></div>:graph&&network&&map&&<>
   ...
   {graph.inference&&<KnowledgeInferences key={`${graph.identity}:${graph.revision}:${graph.mode}:${graph.asOf}:${trail?.index??0}`} graph={graph} value={graph.inference} shown={showImplied} toggle={shown=>setInferenceChoice({graph,shown})} inspect={(id,factIds)=>{setCommunity(null);setSelection({graph,id,factIds});requestAnimationFrame(()=>{inspector.current?.focus({preventScroll:true});inspector.current?.scrollIntoView({block:'nearest'});});}}/>}
...
function EntityInspector({api,graph,selected,reference,focusFacts,focusLabel,available,choose}:{api:ApiClient;graph:Knowledge;selected:string;reference?:PathEntity;focusFacts?:number[];focusLabel:string;available:boolean;choose:(id:string)=>void}){
 ...
 return <><div className="knowledge-entity-title"><span>{entity?.kind??'Unclassified entity'}{entity?` · ${entity.kindStatus}`:''}</span><h2>{entity?.label??reference?.label??selected}</h2>{entity&&<p>{entity.claims} supporting claims in the displayed view</p>}</div>
  ...
  {!available?<p>This server does not offer entity inspection.</p>:!result?<p role="status">Checking supporting records…</p>:result.error?<div role="alert"><p>{result.error}</p><button className="btn quiet" onClick={()=>setAttempt(value=>value+1)}>Retry inspection</button></div>:detail&&<>
   ...
   <EntityInferences key={`${graph.identity}:${graph.revision}:${selected}:${attempt}`} edges={detail.follows} total={detail.coverage.counts.follows_total}/>
