...
export function KnowledgeView({api,detailsAvailable,statusAvailable}:{api:ApiClient;detailsAvailable:boolean;statusAvailable:boolean}){
 const [mode,setMode]=useState<KnowledgeMode>('current'),[attempt,setAttempt]=useState(0);
 const request=useMemo(()=>({api,mode,attempt,statusAvailable}),[api,mode,attempt,statusAvailable]);
 ...
 useEffect(()=>{
  ...
  Promise.all([api.request<unknown>('/v1/graph/knowledge?'+new URLSearchParams({status:mode,limit:'150',attribute_limit:'1000'}),{signal,cache:'no-store'}),statusAvailable?api.request<unknown>('/v1/status',{signal,cache:'no-store'}).then(verifiedSpace):Promise.resolve(undefined)])
   .then(([value,space])=>parseKnowledge(value,mode,space)).then(data=>{if(!controller.signal.aborted)setSnapshot({request,result:{data}});})
 ...
 const visible=graph?.entities.filter(entity=>`${entity.label} ${entity.key} ${entity.kind??''}`.toLocaleLowerCase().includes(query.toLocaleLowerCase()))??[];
 const choose=(id:string)=>{if(graph)setSelection({graph,id});};
 const edges=useMemo(()=>network?.edges.slice(0,1000)??[],[network]);
 return <section className="knowledge-workspace" aria-label="Knowledge workspace">
  <div className="knowledge-toolbar"><label>Claims <select value={mode} onChange={event=>setMode(event.target.value as KnowledgeMode)}>{KNOWLEDGE_MODES.map(value=><option key={value} value={value}>{value==='current'?'Current':value==='history'?'History':value==='proposed'?'Awaiting review':'Include excluded'}</option>)}</select></label><button className="btn quiet" onClick={()=>setAttempt(value=>value+1)}>Refresh graph</button><span>Relationships come from recorded claims.</span></div>
  {!result?<p role="status">Reading this space’s knowledge…</p>:result.error?<div role="alert"><h2>Knowledge could not be loaded</h2><p>{result.error}</p><button className="btn quiet" onClick={()=>setAttempt(value=>value+1)}>Try again</button></div>:graph&&network&&<>
   ...
   {!graph.entities.length?<div className="knowledge-empty"><h2>No entities in this view</h2><p>Store and extract claims, or choose another claim view. {graph.coverage.truncated?'This read was limited, so absence is not conclusive.':''}</p></div>:<div className="knowledge-layout">
    ...
    <div className="knowledge-map"><EvidenceNetworkCanvas nodes={network.nodes} edges={edges} groupColors={false} selectedNode={selected} selectedEdge={null} selectNode={choose} selectEdge={id=>{const edge=edges.find(edge=>edge.id===id);if(edge)choose(edge.source);}} reset={()=>setSelection(null)}/>{edges.length<network.edges.length&&<p className="knowledge-map-limit">Map draws {edges.length} of {network.edges.length} returned relationships. Select an entity to inspect its recorded support.</p>}</div>
