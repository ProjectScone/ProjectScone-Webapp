...
export function ConversationPage({api,enabled}:{api:ApiClient;enabled:boolean}){
  ...
  useEffect(()=>{
    const controller=new AbortController();lifetime.current=controller;setCap(null);setItems([]);setError('');setAfter(null);
  ...
  async function more(){
    ...
    try{const page=sessionPage(await api.request('/v1/conversations?limit=100&after='+encodeURIComponent(after),{signal:controller.signal}));if(!controller.signal.aborted){setItems(previous=>[...previous,...page.items.filter(item=>!previous.some(old=>old.session_id===item.session_id))]);setAfter(page.next_after);}}
