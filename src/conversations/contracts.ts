...
export interface ConversationSession {session_id:string;space:string;state:SessionState;revision:number;created_at:string;active_request_id?:string|null}
...
export interface TurnReceipt {request_id:string;status:'pending'|'completed'|'failed'|'interrupted';result?:TurnResult}
...
export function capabilities(value:unknown):Capabilities{
  ...
  if(v.schema_version!==1||typeof v.text_configured!=='boolean'||v.reply_transport!=='poll'||v.reply_replay!=='process_lifetime')throw Error('This conversation service has an unsupported capability contract.');
...
export function session(value:unknown):ConversationSession{
  ...
  return {session_id:identifier(v.session_id),space:text(v.space),state:v.state as SessionState,revision:integer(v.revision),created_at:text(v.created_at),active_request_id:v.active_request_id==null?null:identifier(v.active_request_id)};
}
...
export function turnReceipt(value:unknown):TurnReceipt{
  ...
  const result:TurnReceipt={request_id:identifier(v.request_id),status:v.status as TurnReceipt['status']};
