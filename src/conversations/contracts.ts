...
import {readPerformance,type TurnPerformance} from './performance.ts';
...
export interface TurnReceipt {request_id:string;status:'pending'|'completed'|'failed'|'interrupted'|'cancelled';result_state:'available'|'forgotten'|'unavailable'|'unreadable';result?:TurnResult;performance?:TurnPerformance}
...
export function turnReceipt(value:unknown):TurnReceipt{
  ...
  const performance=readPerformance(v.performance);
