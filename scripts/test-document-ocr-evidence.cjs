...
async function fixture(t,{mobile=false,regions=25,advertised=true,dimension=null}={}){
 ...
 const content=observed.map(r=>r.text).join(' '),episode={episode_id:1,kind:'file',content,source:'attachment:'+original.attachment_id,created_at:'2026-09-11T00:00:00Z',metadata:{document_filename:'scan.pdf',document_original:original.attachment_id,document_manifest:manifest.attachment_id,document_format:'pdf'},attachments:[original,manifest]};
 const evidence={original,manifest,filename:'scan.pdf',format:'pdf',parser:'fixture-ocr',segments:[{locator:'page:3',text:content,metadata:{page:'3',width_points:dimension||'612',height_points:dimension||'792',rotation:'90',extraction:'ocr',ocr_engine:'fixture-local'},regions:observed}]};
 ...
 await page.goto(`http://127.0.0.1:${server.address().port}/memory/sources/1?space=alpha`);await page.getByRole('heading',{name:'scan.pdf',exact:true}).waitFor();
...
for(const mobile of [false,true])test(`saved OCR inspection is lazy, paged and read-only, mobile=${mobile}`,async t=>{
 ...
 await q.getByRole('button',{name:'Read PDF extraction',exact:true}).click();await q.getByText('PDF page 3 · 1 of 1 retained',{exact:true}).waitFor();
...
test('finite extreme page dimensions cannot overflow the normalized region map',async t=>{
 const {q}=await fixture(t,{dimension:'1e307'});await q.getByRole('button',{name:'Read PDF extraction',exact:true}).click();await q.getByText('PDF page 3 · 1 of 1 retained',{exact:true}).waitFor();
