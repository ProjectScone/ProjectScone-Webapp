...
for(const mode of ['desktop','mobile','corrupt','forgotten','cancel','legacy','malformed'])test(`transcript and checked audio: ${mode}`,async t=>{
 ...
 const segments=Array.from({length:21},(_,i)=>({text:`Café transcript segment ${i+1}.`,locator:`audio:0/segment:${i+1}/seconds:${(i/10).toFixed(2)}-${(i/10+0.05).toFixed(2)}`,metadata:{extraction:'transcription',audio_stream:'0',start_seconds:(i/10).toFixed(2),end_seconds:(i/10+0.05).toFixed(2)}}));
 ...
 if(mode==='malformed'){await panel.getByRole('alert').waitFor();assert.equal(await panel.locator('audio').count(),0);assert.equal(audioReads,0);return;}
