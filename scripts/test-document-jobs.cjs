...
for(const mobile of [false,true])test(`durable imports survive navigation with explicit controls, mobile=${mobile}`,async t=>{
 ...
 await region.locator('input[type=file]').setInputFiles({name:'notes.txt',mimeType:'text/plain',buffer:Buffer.from('Ada maintains the observatory.')});
 ...
 assert.equal(writes.filter(value=>value==='/v1/document-jobs').length,1);
 ...
 await region.getByRole('button',{name:'Verify source for notes.txt'}).click();
 ...
 await page.reload();await region.getByRole('button',{name:'Verify source for notes.txt'}).waitFor();assert.equal(writes.length,2);
 ...
 assert.equal(await region.getByRole('button',{name:'Resume notes.txt'}).count(),0);
 await region.getByRole('button',{name:'Verify source for notes.txt'}).click();
 ...
 await region.getByRole('button',{name:'Refresh import history'}).click();
 await region.getByRole('button',{name:'Resume notes.txt'}).click();
 await region.getByRole('button',{name:'Cancel notes.txt'}).click();
 ...
 assert.equal(await region.getByRole('button',{name:'Resume notes.txt'}).isDisabled(),true);
